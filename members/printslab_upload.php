...
if(!$global_settings["prints_lab"]){header("location:profile_home.php");}
...
<h1><?=word_lang("prints lab")?> &mdash; <?=word_lang("upload")?> <?=word_lang("photos")?></h1>

<div class="subheader"><?=word_lang("select files")?>. (*.jpg <  <?=$global_settings["prints_lab_filesize"]?>M)</b></div>
...
<noscript><link rel="stylesheet" href="../admin/plugins/jquery-file-upload/css/jquery.fileupload-ui-noscript.css"></noscript>
  ...
  <!-- The file upload form used as target for the file upload widget -->
    <form id="fileupload" action="<?=site_root?>/members/printslab_preupload.php" method="POST" enctype="multipart/form-data">
        <!-- Redirect browsers with JavaScript disabled to the origin page -->
        <noscript><input type="hidden" name="redirect" value="<?=site_root?>/members/printslab_preupload.php"></noscript>
...
$(function () {
    ...
    $('#fileupload').fileupload({
        ...
        maxFileSize: <?=$global_settings["prints_lab_filesize"]*1000000?>,
            acceptFileTypes: /(\.|\/)(jpe?g)$/i,
            process: [
                {
                    action: 'load',
                    fileTypes: /^image\/(jpeg)$/,
                    maxFileSize: <?=$global_settings["prints_lab_filesize"]*1000000?> // 20MB
                },
                {
                    action: 'save'
                }
...
<form method="post" Enctype="multipart/form-data" action="printslab_upload2.php" name="uploadform">
<div class="subheader"><?=word_lang("select gallery")?></div>

<div class="subheader_text">
	<select name="gallery_id" style="width:250px" class="ibox form-control">
		<?
		$sql="select id,title from galleries where user_id='".(int)$_SESSION["people_id"]."' order by title";
		$rs->open($sql);
		while(!$rs->eof)
		{
			$sel="";
	...
	<div class="form_field">
		<input type="submit" name="subm" class="isubmit" value="<?=word_lang("next step")?>">
	</div>
