...
{
	$mass=explode("-",result($_GET["shopid"]));
    $product_type=$mass[0];
    $id=(int)$mass[1];
	...
	if($_GET["art"]=="request")
	{
		...
		{
			$sql="select * from credits_list where id_parent=".$id;
			$rs->open($sql);
			if(!$rs->eof)
			{
    			...
    			$sql="select email from users where login='".$rs->row["user"]."'";
    			$ds->open($sql);
    			if(!$ds->eof)
    			{
		...
		{
			$sql="select * from subscription_list where id_parent=".$id;
			$rs->open($sql);
			if(!$rs->eof)
			{
				$product_total=100*$rs->row["total"];
		...
		{
			$sql="select * from orders where id=".(int)$id;
			$rs->open($sql);
			if(!$rs->eof)
			{
				$product_total=100*$rs->row["total"];
    			...
    			$product_city=$rs->row["billing_city"];
    			$product_country=$rs->row["billing_country"];
    			$sql="select email from users where id_parent=".$rs->row["user"];
    			$ds->open($sql);
    			if(!$ds->eof)
    			{
    				$product_email=$ds->row["email"];
		...
		echo($product_firstname.";".$product_lastname.";".$product_address.";".$product_zip.";".$product_city.";".$product_country.";".$product_email.";".$product_total.";;".$_SERVER["REMOTE_ADDR"].";;;;;".$currency_code1);
		exit();
	}
	...
	if($_GET["art"]=="result")
	{
		if($_GET["errcod"]==0)
		{   						
			$transaction_id=transaction_add("inetcash",result($_GET["belegnr"]),result($product_type),$id);
