...
if(isset($_GET["id"]))
{
	...
	$sql="select id_parent,title,description,keywords,userid,duration,format,source,holder,model,free,category2,category3,google_x,google_y,adult,rights_managed from audio where id_parent=".(int)$_GET["id"]." and (userid=".(int)$_SESSION["people_id"]." or author='".result($_SESSION["people_login"])."')";
	$rs->open($sql);
	if(!$rs->eof)
	{
		$title=$rs->row["title"];
...
<form method="post" Enctype="multipart/form-data" action="upload_audio.php?d=4<?if(isset($_GET["id"])){echo("&id=".$_GET["id"]);}?>"  id="uploadform" name="uploadform"  onSubmit="return my_form_validate();">
...
<?if($global_settings["royalty_free"]){?>
	<div id="box_license1" style="display:<?if($rights_managed==0){echo("block");}else{echo("none");}?>">
		<div class="form_field">
			<?=files_upload_form($id,"audio",false)?>
		</div>
	</div>
	<?}?>
	...
	<div id="box_license2" style="display:<?if($rights_managed>0){echo("block");}else{echo("none");}?>">
		<?=rights_managed_upload_form("audio",$rights_managed,$id,false)?>
	</div>
	<?}?>
</div>	
...
<?if(isset($fields_show["description"])){?>
	<div class="form_field">
		...
		<textarea class='ibox form-control' name="description" id="description" style="width:450px;height:70px"><?=$description?></textarea>
	</div>
<?}?>
...
<?if(isset($fields_show["copyright holder"])){?>
	<div class="form_field">
		...
		<input class='ibox form-control' name="holder" id="holder" value="<?=$holder?>" type="text" style="width:200px">
	</div>
<?}?>
