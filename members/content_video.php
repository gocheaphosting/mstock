...
{
	$sql="select id_parent,title,data,description,viewed,author,keywords,userid,usa,duration,format,ratio,rendering,holder,frames,content_type,free,downloaded,rating,model,server1,category2,category3,google_x,google_y,url,rights_managed,vote_like,vote_dislike,contacts,exclusive,content_type from videos where published=1 and id_parent=".(int)$id_parent;
	$rs->open($sql);
	if(!$rs->eof)
	{
		...
		//Video preview URL
		$preview=show_preview($rs->row["id_parent"],"video",2,0,$rs->row["server1"],$rs->row["id_parent"]);
		$boxcontent=str_replace("{PREVIEW}",$preview,$boxcontent);
		
		
		$video=show_preview($rs->row["id_parent"],"video",2,1,$rs->row["server1"],$rs->row["id_parent"]);
		...
		$boxcontent=str_replace("{SHARE_IMAGE}",urlencode(surl.show_preview($rs->row["id_parent"],"video",1,1,$rs->row["server1"],$rs->row["id_parent"])),$boxcontent);
		...
		if($global_settings["download_sample"] and !preg_match("/icon_video/",$video))
		{
			$flag_downloadsample=true;
		}
		$boxcontent=format_layout($boxcontent,"downloadsample",$flag_downloadsample);
	
		//$photo_preview=show_preview($rs->row["id_parent"],"video",3,1,$rs->row["server1"],$rs->row["id_parent"]);
		//$boxcontent=str_replace("{DOWNLOADSAMPLE}",$photo_preview,$boxcontent);
		...
		else
		{
			foreach ($remote_previews as $key => $value) 
			{
				...
				{
					if(preg_match("/.jpg$|.jpeg$|.png$|.gif$/i",$key) and !preg_match("/thumb100/i",$key)) 
					{
						...
						while(!$dr->eof)
						{
							...
							while(!$ds->eof)
							{
								...
								else
								{
									...
									{
										$size=strval(float_opt(filesize($DOCUMENT_ROOT.server_url($rs->row["server1"])."/".$folder."/".$ds->row["url"])/(1024*1024),3))." Mb.";
									}
									else
									{
