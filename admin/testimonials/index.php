...
while(!$rs->eof)
{
	$cl3="";
	$cl_script="";
	if(isset($_SESSION["user_testimonials_id"]) and !isset($_SESSION["admin_rows_testimonials".$rs->row["id_parent"]]) and $rs->row["id_parent"]>$_SESSION["user_testimonials_id"])
	{
		$cl3="success";	
		$cl_script="onMouseover=\"deselect_row('testimonials".$rs->row["id_parent"]."')\"";
	}
?>
<tr id="testimonials<?=$rs->row["id_parent"]?>" class='<?if($tr%2==0){echo("snd");}?> <?=$cl3?>' valign="top" <?=$cl_script?>>
