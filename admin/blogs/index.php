...
while(!$rs->eof)
{
	$cl3="";
	$cl_script="";
	if(isset($_SESSION["user_blog_id"]) and !isset($_SESSION["admin_rows_blog".$rs->row["id_parent"]]) and $rs->row["id_parent"]>$_SESSION["user_blog_id"])
	{
		$cl3="success";	
		$cl_script="onMouseover=\"deselect_row('blog".$rs->row["id_parent"]."')\"";
	}
?>
<tr id="blog<?=$rs->row["id_parent"]?>" class='<?if($tr%2==0){echo("snd");}?> <?=$cl3?>' valign="top" <?=$cl_script?>>
