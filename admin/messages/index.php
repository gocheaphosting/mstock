...
<a class="btn btn-success toright" href="" id="currency_add"><i class="icon-envelope icon-white"></i> <?=word_lang("new message")?></a>

<div style='display:none'>
		
		<div id='new_box'>
		<div class="modal_header"><?=word_lang("new message")?></div>
		
		<form method="post" action="send_message.php" style="margin:0px">
			<div class="form_field">
				<span><b><?=word_lang("To user")?> (login)</b></span>
			...
			<div class="form_field">
				<input class="btn btn-primary" type="submit" value="<?=word_lang("send")?>">
...
while(!$rs->eof)
{
	$cl3="";
	$cl_script="";
	if(isset($_SESSION["user_messages_id"]) and !isset($_SESSION["admin_rows_messages".$rs->row["id_parent"]]) and $rs->row["id_parent"]>$_SESSION["user_messages_id"])
	{
		$cl3="success";	
		$cl_script="onMouseover=\"deselect_row('messages".$rs->row["id_parent"]."')\"";
	}
?>
<tr id="messages<?=$rs->row["id_parent"]?>" class='<?if($tr%2==0){echo("snd");}?> <?=$cl3?>' valign="top" <?=$cl_script?>>
